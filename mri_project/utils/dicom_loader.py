...
def load_dicom_volume(
    ...
    if not dcm_files:
        raise FileNotFoundError(f"В папке {folder} не найдено .dcm файлов")
    ...
    print(f"✅ Загружено {len(slices)} срезов из {len(dcm_files)} файлов | "
...
def predict_volume(
    ...
    for start in range(0, n, batch_size):
        ...
        preds = model.predict(batch, verbose=0)     # (B, H, W, 1)
